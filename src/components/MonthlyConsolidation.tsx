          ...
          >
            <div className="flex items-start sm:items-center gap-2.5">
              <div
                className={`p-2 rounded-lg shrink-0 ${
                  daysAboveTarget.length > 0 ? 'bg-rose-600 text-white shadow-xs' : 'bg-emerald-600 text-white shadow-xs'
                }`}
              >
                {daysAboveTarget.length > 0 ? (
              ...
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  <h4 className="text-xs font-bold uppercase tracking-wide">
                    {daysAboveTarget.length > 0
                      ? `${daysAboveTarget.length} ${
                          daysAboveTarget.length === 1 ? 'Dia Ultrapassou' : 'Dias Ultrapassaram'
                        } a Meta de ${metaLimit.toFixed(1)}%`
                      : `Operação 100% Dentro da Meta (≤ ${metaLimit.toFixed(1)}%)`}
                  </h4>
                  {filters?.metaStatus && filters.metaStatus !== 'all' && (
                    <span className="text-[10px] px-2 py-0.5 rounded-full font-extrabold bg-white border border-slate-200 text-slate-700 shadow-2xs">
                      Filtro: {filters.metaStatus === 'outside' ? 'Apenas Fora da Meta' : 'Apenas Dentro da Meta'}
                    </span>
                  )}
                </div>
                <p className="text-[11px] opacity-90 mt-0.5">
                  {daysAboveTarget.length > 0
                    ? `Identificamos ${daysAboveTarget.length} dia(s) com taxa de retenção > ${metaLimit.toFixed(1)}%. Clique em um dia para inspecionar os detalhes:`
                    : `Todos os dias do período filtrado mantiveram a retenção dentro do limite operacional estabelecido.`}
                </p>
              </div>
            </div>
            ...
            {daysAboveTarget.length > 0 && (
              <div className="flex items-center gap-1.5 flex-wrap">
                {daysAboveTarget.slice(0, 5).map((d) => (
                  <button
                    key={d.key}
                    type="button"
                    onClick={() => {
                    ...
                    className="px-2.5 py-1 rounded-lg bg-white hover:bg-rose-100/90 text-rose-900 border border-rose-300 text-xs font-bold shadow-2xs transition cursor-pointer flex items-center gap-1 hover:scale-105 active:scale-95"
                    title={`Inspecionar dia ${d.fullDate || d.label} (${d.percentualRetida}% - ${d.qtdRetida} cx retidas de ${d.qtdPuxada} cx)`}
                  >
                    <span>📅 {d.label}</span>
                    <span className="text-rose-600 font-black">({d.percentualRetida.toFixed(1)}%)</span>
                  </button>
                ))}
                ...
                {onFilterChange && filters && (
                  <button
                    ...
                    onClick={() => {
                      onFilterChange({
                        ...filters,
                        metaStatus: filters.metaStatus === 'outside' ? 'all' : 'outside',
                      });
                    }}
                    className={`px-3 py-1 rounded-lg text-xs font-bold shadow-xs transition cursor-pointer flex items-center gap-1 ${
                  ...
                  >
                    ...
                    {filters.metaStatus === 'outside' ? 'Ver Todos os Dias' : 'Filtrar Dias > 5%'}
                  </button>
                )}
              </div>
            )}
